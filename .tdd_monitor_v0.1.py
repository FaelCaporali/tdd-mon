...
def read_tests(dir_path: str, test_file: str | bool) -> list[dict]:
    ...
    if test_file is False:
        ...
        for file in docs:
            readings.append(
                {"file": file, "content": read_file(f"{dir_path}/{file}")}
            )
    else:
        readings.append(
            {
...
def do_the_test(file_path: str):
    ...
    pytest.main([file_path, "-n", "auto"])


def monitor(base_file: str, file_path: str, tests_source: str = None):
    tests_source = file_path if tests_source is None else tests_source
    ...
    if check is not False:
        base_file = check
        do_the_test(tests_source)

    recursively_call = threading.Timer(
        2, monitor, [base_file, file_path, tests_source]
    )
...
if __name__ == "__main__":
    ...
    try:
        ...
        tests_source = (
            tests_dir if test_file is False else f"{tests_dir}/{test_file}"
        )
        monitor(first_sc_read, arguments[1], tests_source)
